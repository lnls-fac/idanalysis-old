...
def init_objects(phase, gap, mr=1.25):
    """."""
...
def run_fieldmap():
    ...
    epu, fmap = init_objects(phase='+00.00', gap='32.5', mr=1.25)
    ...
    scale = minscale
    epu, _ = init_objects(phase='+00.00', gap='32.5', mr=1.25*-1*scale)
    z = np.arange(-1800, 1802, 3).tolist()
    y = np.linspace(-4, 4, 3).tolist()
    x = np.linspace(-6, 6, 3).tolist()
    field = epu.save_fieldmap(
        filename='testmap.dat', x_list=x, y_list=y, z_list=z)


def generate_kickmap(posx, posy):
    idkickmap = IDKickMap()
    fmap_fname = 'testmap.dat'
    idkickmap.fmap_fname = fmap_fname
...
if __name__ == "__main__":
    """."""
    posx = np.arange(-5, +5, 1) / 1000  # [m]
    posy = np.arange(-4, +4, 1) / 1000  # [m]

    # run_fieldmap()
    generate_kickmap(posx, posy)
