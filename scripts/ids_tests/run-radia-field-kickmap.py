...
def create_model(phase, width):
    ...
    kyma = Kyma22(nr_periods=nr_periods, block_shape=block_shape)
    kyma.dp = phase

    return kyma


def create_path(phase, width=None):
    ...
    if width is not None:
        ...
        fpath = fpath.replace('data/', 'data/phase_{}/'.format(
            phase_str))
    return fpath
...
def generate_kickmap(gridx, gridy, phase, radia_model, max_rz, width):
...
def get_field_on_trajectory(kyma, data, max_rz, rx_init=0):
...
def save_data(data, width):
    """."""
    phase = data['phase']
    fpath = create_path(phase, width)
    fname = fpath + f'field_data_kyma22_width_{width}'
    save_pickle(data, fname, overwrite=True, makedirs=True)


def plot_field_on_axis(data, width):
    phase = data['phase']
    fpath = create_path(phase, width)
    by = data['onaxis_by']
    bx = data['onaxis_bx']
...
def plot_field_roll_off(data, width):
    phase = data['phase']
    by = data['rolloff_by']
    rx = data['rolloff_rx']
...
def plot_rk_traj(data, rx_init=0, width=36):
    phase = data['phase']
    colors = ['b', 'g', 'y', 'C1', 'r', 'k']
    fpath = create_path(phase, width)
...
def run_calc_fields(phase, rx_init, width):

    kyma = create_model(phase, width=width)
    ...
    data = get_field_on_trajectory(
        kyma=kyma, data=data, max_rz=max_rz, rx_init=rx_init)

    # --- save data
    save_data(data, width)
...
def run_generate_kickmap(kyma=None,
    ...
    generate_kickmap(
        gridx=gridx, gridy=gridy, phase=phase, radia_model=kyma, max_rz=max_rz, width=width)

    return kyma


def run_plot_data(phase, rx_init, width):

    fpath = create_path(phase)
    fname = fpath + f'field_data_kyma22_width_{width}.pickle'
    data = load_pickle(fname)

    plot_field_on_axis(data=data, width=width)
    plot_field_roll_off(data=data, width=width)
    plot_rk_traj(data=data, rx_init=rx_init, width=width)
...
if __name__ == "__main__":
    ...
    kyma, max_rz = run_calc_fields(phase, rx_init, width)
    # run_plot_data(phase=phase, rx_init=rx_init, width=width)
    # kyma = run_generate_kickmap(
        # kyma=kyma, phase=phase, max_rz=max_rz, width=width)
