...
def check_kick_at_plane(gap, width, rescale_kicks):
    ...
    for x0 in rx0:
        ...
        rxf_trk, pxf_trk = x[idx_dif+1], px[idx_dif+1]
        ryf_trk, pyf_trk = y[idx_dif+1], py[idx_dif+1]
    ...
    plt.plot(
        1e3*rx0, 1e6*(rxf_trk - rx0), 'o', color='C1', label='Pos X  tracking')
