...
def calc_idkmap_kicks(plane_idx=0, plot_flag=False, idkmap=None):
  ...
  if plot_flag:
    ...
    plt.plot(1e3*rx0, 1e6*pyf, label = 'Kick Y', color='C0')
...
if __name__ == '__main__':
  fname = "results/ID3969/kickmap-ID3969.txt"
  idconfig = fname[8:14]
  ...
  plt.plot(1e3*rx0, 1e6*pyf,'-', color='C0', label = 'Kick Y - kickmap')
  plt.plot(1e3*rx0, 1e6*pxf_array,'o', color='C1', label = 'Kick X - tracking')
  plt.plot(1e3*rx0, 1e6*pyf_array,'o', color='C0', label = 'Kick Y - tracking')
  plt.xlabel('init rx [mm]')
  plt.ylabel('final px [urad]')
  plt.title('Kicks')
  plt.legend()
